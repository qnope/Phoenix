...
class DrawInformationsAllocatorImpl {
  public:
    ...
    DrawInformations allocate(const std::vector<Complete3dVertex> &vertices, const std::vector<uint32_t> &indices) {
        ...
        return DrawInformations(indexBufferInfo.buffer, vertexBufferInfo.buffer, indexBufferInfo.size,
                                indexBufferInfo.offset, vertexBufferInfo.offset);
    }
...
DrawInformationsAllocator::DrawInformationsAllocator(Device &device) :
    m_vertexAndIndexBufferListPimpl{std::make_unique<details::DrawInformationsAllocatorImpl>(device)} {}

DrawInformationsAllocator::~DrawInformationsAllocator() = default;

DrawInformations DrawInformationsAllocator::allocate(const std::vector<Complete3dVertex> &vertices,
                                                     const std::vector<uint32_t> &indices) {
    return m_vertexAndIndexBufferListPimpl->allocate(vertices, indices);
}

void DrawInformationsAllocator::flush(vk::PipelineStageFlags nextPipeline, vk::AccessFlags nextAccess) noexcept {
