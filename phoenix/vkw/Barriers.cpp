...
void applyBarrier(vk::CommandBuffer cmd, FullBarrier) {
    auto fullAccess = vk::AccessFlagBits::eMemoryWrite | vk::AccessFlagBits::eMemoryRead;
    cmd.pipelineBarrier(vk::PipelineStageFlagBits::eAllCommands, vk::PipelineStageFlagBits::eAllCommands,
                        vk::DependencyFlags(), vk::MemoryBarrier(fullAccess, fullAccess), {}, {});
}
...
void applyBarrier(vk::CommandBuffer cmd, MemoryBarrier barrier) {
    ...
    cmd.pipelineBarrier(barrier.srcStage, barrier.dstStage, vk::DependencyFlags(), memoryBarrier, {}, {});
}

void applyBarrier(vk::CommandBuffer cmd, LayoutTransitionBarrier barrier) {
    vk::ImageMemoryBarrier memoryBarrier(barrier.srcAccess, barrier.dstAccess, barrier.oldLayout, barrier.newLayout,
                                         VK_QUEUE_FAMILY_IGNORED, VK_QUEUE_FAMILY_IGNORED, barrier.image,
                                         barrier.range);

    cmd.pipelineBarrier(barrier.srcStage, barrier.dstStage, vk::DependencyFlagBits(), {}, {}, memoryBarrier);
}
...
void applyBarrier(vk::CommandBuffer cmd, LayoutTransitionTransferDstToSampledBarrier barrier) {
    vk::ImageMemoryBarrier memoryBarrier(vk::AccessFlagBits::eTransferWrite, vk::AccessFlagBits::eShaderRead,
                                         vk::ImageLayout::eTransferDstOptimal, vk::ImageLayout::eShaderReadOnlyOptimal,
                                         VK_QUEUE_FAMILY_IGNORED, VK_QUEUE_FAMILY_IGNORED, barrier.image,
                                         barrier.range);
    cmd.pipelineBarrier(vk::PipelineStageFlagBits::eTransfer, barrier.dstStage, vk::DependencyFlags(), {}, {},
                        memoryBarrier);
}

void applyBarrier(vk::CommandBuffer cmd, LayoutTransitionTransferSrcToSampledBarrier barrier) {
    vk::ImageMemoryBarrier memoryBarrier(vk::AccessFlags(), vk::AccessFlagBits::eShaderRead,
...
void applyBarrier(vk::CommandBuffer cmd, LayoutTransitionUndefinedToTransferDstBarrier barrier) {
    vk::ImageMemoryBarrier memoryBarrier(vk::AccessFlags(), vk::AccessFlagBits::eTransferWrite,
