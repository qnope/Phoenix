...
class MemoryTransfer {
  ...
  public:
    MemoryTransfer(Device &device) noexcept :
        ...
        m_commandPool{device.getHandle(), m_queue.getIndexFamily(), true, false} {}
    ...
    void copyBufferRange(const B1 &src, const B2 &dst, vk::BufferCopy range) {
        typed_static_assert_msg(doesBufferSupport<vk::BufferUsageFlagBits::eTransferSrc>(src),
                                "Src Buffer must be a transferable source");

        typed_static_assert_msg(doesBufferSupport<vk::BufferUsageFlagBits::eTransferDst>(dst),
                                "Dst Buffer must be a transferable destination");

        typed_static_assert_msg(dst.type == src.type, "Src and dst Buffer must be of the same type");

        assert(src.sizeInBytes() >= range.srcOffset + range.size);
        assert(dst.sizeInBytes() >= range.dstOffset + range.size);
        getCurrentCommandBuffer().copyBuffer(src.getHandle(), dst.getHandle(), range);
    }
    ...
    template <typename B, typename I>
    void copyBufferToImage(const B &buffer, const I &image, vk::BufferImageCopy range) {
        typed_static_assert_msg(doesBufferSupport<vk::BufferUsageFlagBits::eTransferSrc>(buffer),
                                "Src Buffer must be a transferable source");

        typed_static_assert_msg(doesImageSupport<vk::ImageUsageFlagBits::eTransferDst>(image),
                                "image must be a transferable destination");

        getCurrentCommandBuffer().copyBufferToImage(buffer.getHandle(), image.getHandle(),
                                                    vk::ImageLayout::eTransferDstOptimal, range);
    }

    template <typename I>
    void generateMipmap(const I &image) {
        typed_static_assert_msg(doesImageSupport<vk::ImageUsageFlagBits::eTransferSrc>(image),
        ...
        for (uint32_t i = 1; i < image.getMipLevels(); ++i) {
            ...
            vk::ImageBlit blitter(srcLayer, {vk::Offset3D{}, srcOffset}, dstLayer, {vk::Offset3D{}, dstOffset});
            cmd.blitImage(handle, vk::ImageLayout::eTransferSrcOptimal, handle, vk::ImageLayout::eTransferDstOptimal,
                          blitter, vk::Filter::eLinear);
    ...
    void applyBarrier(Barrier barrier) { ::phx::applyBarrier(getCurrentCommandBuffer(), barrier); }

    void applyMemoryBarrier(vk::PipelineStageFlags dstStage, vk::AccessFlags dstMasks) {
    ...
    auto to(Buffer<T, enums...> &buffer) {
        return MemoryTransferToBuffer{*this, buffer};
    }

  private:
    void allocateCommandBuffers() {
        auto toTuple = [](auto cmdBuffer) { return ltl::tuple_t{false, cmdBuffer}; };

        m_commandBuffers = m_commandPool.allocateCommandBuffer(vk::CommandBufferLevel::ePrimary, 10) |
