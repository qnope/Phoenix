...
template <VkShaderStageFlags _stages, vk::DescriptorType DescriptorType, uint32_t Count, typename... Types>
struct DescriptorBinding {
    template <int binding>
    auto toDescriptorBinding(ltl::number_t<binding>) const noexcept {
        return vk::DescriptorSetLayoutBinding(binding, descriptorType, count.value, stages);
    }
    ...
    static constexpr auto compute_tuple_types() {
        ...
        else if constexpr (DescriptorType == vk::DescriptorType::eCombinedImageSampler) {
...
static constexpr auto is_buffer(vk::DescriptorType type) {
    return type == ltl::AnyOf{vk::DescriptorType::eUniformBuffer, vk::DescriptorType::eStorageBuffer};
}

static constexpr auto is_image(vk::DescriptorType type) {
    return type == ltl::AnyOf{vk::DescriptorType::eCombinedImageSampler, vk::DescriptorType::eSampledImage};
}

template <typename T>
static constexpr auto getInfos(T values) {
    return values([](auto... refs) { return std::array{::phx::getDescriptorInfo(refs)...}; });
}

} // namespace details
template <int bindingIndex, typename Binding>
void writeDescriptorSet(vk::Device device, vk::DescriptorSet set, ltl::number_t<bindingIndex>, Binding binding,
...
using SampledImage2dRgbaSrgbBinding =
    DescriptorBinding<VK_SHADER_STAGE_FRAGMENT_BIT, vk::DescriptorType::eCombinedImageSampler, 1,
