...
static auto getExtensions(SDL_Window *window, bool debug) {
    ...
    SDL_Vulkan_GetInstanceExtensions(window, &extensionsNumber, extensions.data() + baseNumberExtensions);
...
Instance::Instance(SDL_Window *window, bool debug) {
    ...
    vk::ValidationFeatureEnableEXT enables[] = {vk::ValidationFeatureEnableEXT::eGpuAssisted};
    ...
    if (debug) {
        ...
        m_debugMessenger =
            m_handle->createDebugUtilsMessengerEXTUnique(debugMessengerInfo, nullptr, m_dispatchLoaderDynamic);
    }
}

const std::vector<const char *> &Instance::getValidationLayers() const noexcept { return m_validationLayers; }
