...
namespace phx {
template <typename... Args> class RenderPass;
...
public:
  ...
  RenderPass(vk::Device device, ltl::tuple_t<AttachmentDescriptions...> attachments,
    ...
    auto toSubpassDescriptions = [](auto &... xs) {
      return std::array<vk::SubpassDescription, number_subpasses.value>{
          xs.getSubpassDescription()...};
    };
...
private:
  void compileTimeCheck() {
    ...
    auto isSubpassCompatible = [](auto subpass) {
      return decltype_t(subpass)::getMaxAttachmentIndex() <
             RenderPass::number_attachments;
    };
