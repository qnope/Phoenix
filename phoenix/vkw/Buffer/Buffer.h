...
namespace phx {
template <typename T, VkBufferUsageFlags _bufferUsage, VmaMemoryUsage _memoryType>
class Buffer final {
  public:
    ...
    Buffer(Allocator &allocator, vk::DeviceSize size) noexcept :
        ...
        ltl::tie(m_buffer, m_block) = allocator.allocateBuffer(infoBuffer, memoryType);
    }
    ...
    Buffer &operator<<(T x) noexcept {
        ...
        if_constexpr(has_types(x)) {
            auto types = x.types;
            types([this, &x](auto... types) { ((*this << static_cast<const decltype_t(types) &>(x)), ...); });
        }
        else {
            m_block->push_back(x);
        }
    ...
    vk::DeviceSize capacityInBytes() const noexcept { return capacity() * sizeof(T); }
...
template <typename T>
using VertexBuffer = GpuBuffer<T, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT>;

template <typename T>
using IndexBuffer = GpuBuffer<T, VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT>;
...
template <typename B, requires_f(IsBuffer<B>)>
auto getDescriptorInfo(B &b) {
