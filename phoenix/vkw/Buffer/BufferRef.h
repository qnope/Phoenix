...
class BufferRef {
  public:
    ...
    BufferRef(Buffer<T, usage, memUsage> &buffer) {
        typed_static_assert_msg(buffer.type == type, "Buffer must be of the good type");
        static_assert((buffer.bufferUsage & bufferUsage) == bufferUsage, "Buffer must have the good usage");
