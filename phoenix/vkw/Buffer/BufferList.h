...
template <typename BufferInfo> class BufferList;
...
private:
  static vk::BufferCopy prepareCopy(const std::vector<T> &values,
                                    SrcBuffer &src, DstBuffer &dst) noexcept {
  ...
  auto &getCandidateBuffer(std::size_t size,
                           std::vector<Buffer> &buffers) noexcept {
    auto hasEnoughSpace = [size](const auto &buffer) {
    ...
    return buffers.emplace_back(
        m_device.createBuffer<Buffer>(NUMBER_OF_ELEMENTS_BY_BUFFER));
  }
