...
class Image {
  public:
    ...
    static constexpr vk::ImageAspectFlags aspectMask =
        (format == ltl::AnyOf{vk::Format::eD16Unorm, vk::Format::eD32Sfloat}) ? vk::ImageAspectFlagBits::eDepth
                                                                              : vk::ImageAspectFlagBits::eColor;

    Image(vk::Device device, vk::Image image, vk::Extent3D extent, uint32_t mipLevels, uint32_t arrayLayers) noexcept :
        m_device{device}, m_handle{image}, m_extent{extent}, m_mipLevels{mipLevels}, m_arrayLayers{arrayLayers} {}

    Image(Allocator &allocator, uint32_t width, uint32_t height, uint32_t depth, bool withMipmap,
          VmaMemoryUsage memoryUsage) :
        m_allocator{&allocator},
        m_device{allocator.device()}, m_extent{width, height, depth} {
        if (withMipmap) {
        ...
        ltl::tie(m_handle, m_allocation) = allocator.allocateImage(info, memoryUsage);
    }

    Image(Allocator &allocator, uint32_t width, uint32_t height, uint32_t depth, bool withMipmap) noexcept :
        Image(allocator, width, height, depth, withMipmap, VMA_MEMORY_USAGE_GPU_ONLY) {}

    Image(Image &&image) noexcept :
        m_allocator{image.m_allocator}, m_device{image.m_device}, m_handle{image.m_handle}, m_extent{image.m_extent},
        m_mipLevels{image.m_mipLevels}, m_arrayLayers{image.m_arrayLayers}, m_allocation{image.m_allocation} {
    ...
    ImageView<ImageViewType, Format, Usage> createImageView() const noexcept {
        typed_static_assert_msg((isImageTypeCompatibleWithImageViewType<Type, ImageViewType>()),
                                "ImageViewType must be compatible with ImageType");
        return ImageView<ImageViewType, Format, Usage>{m_device, getHandle(), getSubresourceRange()};
    }

    vk::Offset3D extentAsOffset(uint32_t mipLevel) const {
        return {std::max(1, int32_t(m_extent.width >> mipLevel)), std::max(1, int32_t(m_extent.height >> mipLevel)),
...
template <typename I, requires_f(IsImage<I>)>
auto getDescriptorInfo(I &i) {
    return vk::DescriptorImageInfo(i.sampler, i.imageView, vk::ImageLayout::eShaderReadOnlyOptimal);
}
