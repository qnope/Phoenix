...
constexpr auto imageTypeFromImageViewType() {
    if constexpr (type == vk::ImageViewType::e2D)
        return vk::ImageType::e2D;
}

template <typename SampledImage>
class ImageLoader;
...
class ImageLoader<SampledImageRef<Type, Format, _Usage>> {
    static constexpr auto Usage = _Usage | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT;
  ...
  public:
    ...
    SampledImage load(const std::string &path, bool withMipmap, vk::PipelineStageFlags pipelineStage) {
        ...
        auto subResourceRange = vk::ImageSubresourceRange(image.aspectMask, 0, VK_REMAINING_MIP_LEVELS, 0, 1);

        LayoutTransitionUndefinedToTransferDstBarrier toTransferBarrier{image.getHandle(), subResourceRange};
        ...
        if (withMipmap) {
            ...
            LayoutTransitionTransferSrcToSampledBarrier toSampledBarrier{image.getHandle(), pipelineStage,
                                                                         subResourceRange};
            m_memoryTransfer.applyBarrier(toSampledBarrier);
        } else {
            LayoutTransitionTransferDstToSampledBarrier toSampledBarrier{image.getHandle(), pipelineStage,
                                                                         subResourceRange};
            m_memoryTransfer.applyBarrier(toSampledBarrier);
        }
  ...
  private:
    void generateMipmap(const Image &image) {
        ...
        LayoutTransitionTransferDstToSrcBarrier firstLevelBarrier{image.getHandle(), firstLevelRange};
    ...
    ltl::tuple_t<Image, ImageView> &createNewImage(Width width, Height height, bool withMipmap,
                                                   const std::string &path) {
        auto w = width.get();
        auto h = height.get();
        auto image = m_device.createImage<Image>(w, h, 1u, withMipmap);
        auto imageView = image.template createImageView<Type>();
        auto [it, ok] = m_images.emplace(path, ltl::tuple_t<Image, ImageView>{std::move(image), std::move(imageView)});
        assert(ok);
        return it->second;
    }

    ltl::tuple_t<std::size_t, std::size_t> fillBuffer(Buffer &buffer, const std::vector<std::byte> &datas) {
    ...
    auto &getCandidateBuffer(std::size_t size) {
        auto hasEnoughSize = [size](auto &buffer) { return buffer.size() + size < buffer.capacity(); };

        if (auto *buffer = ltl::find_if_ptr(m_buffers, hasEnoughSize)) {
            return *buffer;
        }
        return m_buffers.emplace_back(m_device.createBuffer<Buffer>(std::max(size, MAX_STAGING_BUFFER_SIZE)));
    }
