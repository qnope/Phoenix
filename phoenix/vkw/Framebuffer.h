...
class Framebuffer : public VulkanResource<vk::UniqueFramebuffer> {
  ...
  typed_static_assert_msg(ltl::all_of_type(attachment_types,
...
public:
  ...
  Framebuffer(vk::Device device, vk::RenderPass renderPass, uint32_t width,
              uint32_t height, ltl::tuple_t<AttachmentTypes...> attachments) noexcept
...
LTL_MAKE_IS_KIND(Framebuffer, isFramebuffer);
} // namespace phx
