...
template <typename RP, typename... Operations>
...
class RenderPassWrapper<RenderPass<RPs...>, Operations...> {
    ...
    static constexpr auto all_operations = ltl::type_list_v<const Operations &...>;
    static constexpr auto subpass_types = ltl::filter_type(all_operations, is_subpass);
  ...
  public:
    ...
    RenderPassWrapper(const RenderPass<RPs...> &renderPass, const Operations &... operations) noexcept :
    ...
    auto operator<<(const Operation &operation) const noexcept {
        auto buildNextWrapper = [this, &operation](const auto &... operations) {
            return RenderPassWrapper<RenderPass<RPs...>, Operations..., Operation>{renderPass, operations...,
                                                                                   operation};
        };
...
class RenderPassWrapperWithFramebuffer {
  public:
    RenderPassWrapperWithFramebuffer(const Framebuffer &fb, const RenderPassWrapper &rp) noexcept :
        framebuffer{fb}, renderPassWrapper{rp} {
        typed_static_assert_msg(rp.renderPass.number_attachments == framebuffer.number_attachments,
                                "Framebuffer must have the same attachments number "
                                "as RenderPass attachments.");
    }

    friend auto operator<<(vk::CommandBuffer commandBuffer, const RenderPassWrapperWithFramebuffer &rp) {
...
inline auto operator<<(const RenderPass<RPs...> &renderPass, const T &operation) {
