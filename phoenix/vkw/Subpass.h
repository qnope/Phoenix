...
struct AttachmentReference {
    ...
    constexpr AttachmentReference(Index idx, vk::ImageLayout layout) : attachmentReference{idx.value, layout} {}

    const vk::AttachmentReference attachmentReference;
};

LTL_MAKE_IS_KIND(AttachmentReference, is_attachment_reference, IsAttachmentReference, typename);

template <typename... Args>
class SubpassDescription;

template <typename... Outputs, typename... Inputs, typename... DepthStencil, typename... Preserve>
class SubpassDescription<ltl::tuple_t<Outputs...>, ltl::tuple_t<Inputs...>, ltl::tuple_t<DepthStencil...>,
    ...
    static constexpr auto construct() {
        return [](auto... xs) { return std::array<vk::AttachmentReference, sizeof...(xs)>{xs.attachmentReference...}; };
    }

  public:
    SubpassDescription(ltl::tuple_t<Outputs...> outputs, ltl::tuple_t<Inputs...> inputs,
                       ltl::tuple_t<DepthStencil...> depthStencil, ltl::tuple_t<Preserve...>) :
        m_outputs{outputs(construct())},
        m_inputs{inputs(construct())}, m_depthStencil{depthStencil(construct())}, m_preserve{static_cast<uint32_t>(
                                                                                      Preserve::value)...} {
        compileTimeCheck();
    }
    ...
    auto getSubpassDescription() const {
        ...
        if_constexpr(hasDepthStencil) { description.pDepthStencilAttachment = m_depthStencil.data(); }
  ...
  private:
    constexpr void compileTimeCheck() {
        ...
        typed_static_assert_msg(all_of_type(depth_stencil_type, is_attachment_reference),
                                "DepthStencil must be an AttachmentReference if there is one");

        typed_static_assert_msg(is_number_list_t(preserve_types), "All preserve attachments must be number_t");
...
SubpassDescription(Ts...) -> SubpassDescription<Ts...>;

LTL_MAKE_IS_KIND(SubpassDescription, is_subpass_description, IsSubpassDescription, typename);

struct AbstractSubpass {};

constexpr auto is_subpass = [](auto &&t) { return ltl::is_base_of(ltl::type_v<AbstractSubpass>, FWD(t)); };
template <typename T>
