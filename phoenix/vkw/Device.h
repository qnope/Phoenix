...
class Device final : public VulkanResource<vk::UniqueDevice> {
  public:
    ...
    auto createSemaphore() const noexcept { return getHandle().createSemaphoreUnique(vk::SemaphoreCreateInfo()); }
    ...
    createGraphicPipeline(PipelineLayout pipelineLayout, const RenderPass<RPs...> &renderPass,
                          SubpassIndex subpassIndex, Args... args) const {
        return {getHandle(), std::move(pipelineLayout), renderPass, subpassIndex, std::move(args)...};
    }

    template <typename... Attachments, typename... Subpasses, typename... Dependencies>
    RenderPass<ltl::tuple_t<Attachments...>, ltl::tuple_t<Subpasses...>, ltl::tuple_t<Dependencies...>>
    createRenderPass(ltl::tuple_t<Attachments...> attachments, ltl::tuple_t<Subpasses...> subpasses,
    ...
    template <typename Image, typename... Ts>
    Image createImage(Ts... ts) const {
        return {*m_allocator, ts...};
    }

    Sampler createSampler(vk::Filter filterMinMag, vk::SamplerMipmapMode mipmapMode) const noexcept {
