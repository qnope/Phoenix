...
constexpr auto getFormat(ltl::type_t<glm::vec2>) { return vk::Format::eR32G32Sfloat; }

constexpr auto getFormat(ltl::type_t<glm::vec3>) { return vk::Format::eR32G32B32Sfloat; }

constexpr auto getFormat(ltl::type_t<glm::vec4>) { return vk::Format::eR32G32B32A32Sfloat; }
...
struct Vertex : Types... {
    ...
    static constexpr auto getAttributeDescription(uint32_t binding, Index index) {
        ...
        return vk::VertexInputAttributeDescription(location_indices[index].value, binding, format,
    ...
    static constexpr auto getBindingDescription(ltl::number_t<N> n) {
        ...
        return indexer([n](auto... index) {
            return BindingDescription{n, ltl::type_v<Vertex>, stride, getAttributeDescription(n.value, index)...};
        });
    }

    static constexpr auto compute_location_indices() {
        return ltl::scanl(_((x, y), x + decltype_t(y)::location_size), 0_n, types).pop_back();
    }

    static constexpr auto compute_offset_list() {
        return ltl::scanl(_((x, y), x + decltype_t(y)::byte_number), 0_n, types).pop_back();
    }
...
using Complete3dVertex = Vertex<Position3D, Normal, Tangeant, BiTangeant, TexCoord>;
