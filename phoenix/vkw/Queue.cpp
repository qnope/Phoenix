...
Queue::Queue(vk::Queue queue, uint32_t indexFamily) noexcept : m_queue{queue}, m_indexFamily{indexFamily} { next(); }

uint32_t Queue::getIndexFamily() const noexcept { return m_indexFamily; }

void Queue::present(vk::Semaphore waitSemaphore, vk::SwapchainKHR swapChain, uint32_t imageIndice) const noexcept {
...
void Queue::populateSubmits() noexcept {
    ...
    for (auto [cmdNumber, waitNumber, signalNumber] :
         ltl::zip(m_commandBufferBySubmitNumbers, m_waitSemaphoreBySubmitNumbers, m_signalSemaphoreBySubmitNumbers)) {
        vk::SubmitInfo info;
