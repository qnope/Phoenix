...
class ShaderCompilerIncluder : public shaderc::CompileOptions::IncluderInterface {
  public:
    shaderc_include_result *GetInclude(const char *requested, shaderc_include_type, const char *requesting,
                                       size_t) override;
...
class ShaderCompiler {
  public:
    ...
    std::vector<uint32_t> compile(const std::string &path, shaderc_shader_kind kind) const;
