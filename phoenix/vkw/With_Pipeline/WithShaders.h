...
class WithShaders {
  ...
  public:
    WithShaders(Shaders... shaders) : m_shaders{std::move(shaders)...} { compileTimeCheck(); }
  ...
  private:
    void compileTimeCheck() {
        using namespace ltl;
        typed_static_assert_msg(shader_types.length > 0_n, "You must have shaders within the WithShaders");

        typed_static_assert_msg(all_of_type(shader_types, is_shader_module),
                                "All arguments from WithShaders must be shaders");

        typed_static_assert_msg(count_type(shader_types, type_v<ShaderModule<VertexShaderType>>) <= 1_n &&
