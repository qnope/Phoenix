...
struct StaticViewport : Viewport {
    StaticViewport(Width width, Height height) :
        m_viewport{0.0f, 0.0f, static_cast<float>(width.get()), static_cast<float>(height.get()), 0.0f, 1.0f} {}
...
class WithViewports {
  ...
  private:
    void compileTimeCheck() {
        ...
        typed_static_assert_msg(numberDynamicViewports == numberViewports || numberStaticViewports == numberViewports,
                                "You can not mix static and dynamic viewports.");
        typed_static_assert_msg(all_of_type(viewport_types, is_derived_from(type_v<viewport::Viewport>)),
