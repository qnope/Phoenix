...
struct NormalAttachment : OutputAttachment {
    auto getAttachmentState() {
        ...
        state.colorWriteMask = vk::ColorComponentFlagBits::eA | vk::ColorComponentFlagBits::eB |
...
class WithOutputs {
  ...
  public:
    ...
    WithOutputs(Outputs... outputs) : m_attachmentStates{outputs.getAttachmentState()...} { compileTimeCheck(); }
  ...
  private:
    void compileTimeCheck() {
        using namespace ltl;
        typed_static_assert_msg(number_outputs > 0_n, "If WithOutputs is specified, it must have at least one output");
        typed_static_assert_msg(all_of_type(output_types, is_derived_from(type_v<output::OutputAttachment>)),
                                "Outputs must be derived from OutputAttachment");
    }

  private:
    std::array<vk::PipelineColorBlendAttachmentState, number_outputs.value> m_attachmentStates;
};
