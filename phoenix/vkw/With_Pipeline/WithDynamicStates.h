...
struct WithDynamicStates {
    ...
    constexpr WithDynamicStates(DynamicStates...) {
        using namespace ltl;
        typed_static_assert_msg(numberDynamicStates > 0_n, "If you specify a WithDynamicStates, you must have "
                                                           "at least one dynamic state");

        typed_static_assert_msg(all_of_type(dynamic_state_types, is_derived_from(type_v<dynamic_state::DynamicState>)),
                                "All dynamic states must be DynamicStates values");

        typed_static_assert_msg(count_type(dynamic_state_types, type_v<dynamic_state::DynamicViewport>) <= 1_n &&
...
LTL_MAKE_IS_KIND(WithDynamicStates, is_with_dynamic_states, IsWithDynamicStates, typename);
