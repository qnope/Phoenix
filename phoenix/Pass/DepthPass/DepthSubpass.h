...
auto make_depth_subpass(Device &device, Width width, Height height, const RenderPass &renderPass,
                        const MatrixBufferLayout &layout) {
    auto vertexShader =
        device.createShaderModule<VertexShaderType>("../phoenix/shaders/DepthPass/DepthPass.vert", true);
    auto fragmentShader =
        device.createShaderModule<FragmentShaderType>("../phoenix/shaders/DepthPass/DepthPass.frag", true);
    auto pipelineLayout = device.createPipelineLayout(ltl::tuple_t{MatrixPushConstant{}}, //
                                                      ltl::tuple_t{std::cref(layout)});

    auto vertexBinding = BindingDescription{0_n, ltl::type_v<Complete3dVertex>, Complete3dVertex::stride,
    ...
    return DepthSubpass{device.createGraphicPipeline(
        std::move(pipelineLayout), renderPass, 0_n, WithBindingDescriptions{vertexBinding},
