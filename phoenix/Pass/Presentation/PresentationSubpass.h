...
class PresentationSubpass : AbstractSubpass {
  public:
    ...
    friend vk::CommandBuffer operator<<(vk::CommandBuffer cmdBuffer, const PresentationSubpass &pass) noexcept;
...
auto make_presentation_pipeline(Device &device, const RenderPass &renderPass, const Layout &layout, Width width,
                                Height height) {
    auto vertexShader =
        device.createShaderModule<VertexShaderType>("../phoenix/shaders/PresentationPass/PresentationPass.vert", true);
    ...
    return device.createGraphicPipeline(
        std::move(pipelineLayout), renderPass, 0_n, WithShaders{std::move(vertexShader), std::move(fragmentShader)},
        WithViewports{viewport::StaticViewport{width, height}}, WithScissors{scissor::StaticScissor{width, height}},
        WithOutputs{output::normal_attachment}, vk::PrimitiveTopology::eTriangleStrip);
}
