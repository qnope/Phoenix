...
static auto make_render_pass(PhoenixWindow &phoenixWindow) {
  ...
  return phoenixWindow.getDevice().createRenderPass(attachment, subpass,
                                                    dependency);
}
...
public:
  ...
  const auto &framebuffer() const noexcept {
  ...
  void setSampledImage(std::size_t index, SampledImage2dRgbaSrgbRef ref) {
    ...
    auto pipeline = make_presentation_pipeline(
...
void PresentationRenderPass::setSampledImage(std::size_t index,
                                             SampledImage2dRgbaSrgbRef ref) {
